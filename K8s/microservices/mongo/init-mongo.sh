...
# Create catalogue database and user
mongosh <<EOF
use admin
db.auth('${MONGO_INITDB_ROOT_USERNAME}', '${MONGO_INITDB_ROOT_PASSWORD}')

use ${CATALOGUE_MONGO_DATABASE}
db.createUser({
  user: '${CATALOGUE_MONGO_USER}',
  pwd: '${CATALOGUE_MONGO_PASSWORD}',
  roles: [
    { role: 'readWrite', db: '${CATALOGUE_MONGO_DATABASE}' }
  ]
})
...
print('Catalogue database initialized')
EOF
...
use ${USER_MONGO_DATABASE}
db.createUser({
  user: '${USER_MONGO_USER}',
  pwd: '${USER_MONGO_PASSWORD}',
  roles: [
    { role: 'readWrite', db: '${USER_MONGO_DATABASE}' }
  ]
})
